...
class Fluid {
    ...
    // pee: {255, 215, 0}, {255, 140, 0}, {255, 69, 0}, {128, 0, 0}
...
public:
    Fluid(float WIDTH, float HEIGHT, float cellSpacing, int numParticles, float gravity, float k, float diffusionRatio, float separationInit, float vorticityStrength_, float flipRatio_, float overRelaxation_, float numPressureIters_, tp::ThreadPool& tp)
        : numX(std::floor(WIDTH / cellSpacing)), numY(std::floor(HEIGHT / cellSpacing)), numCells(numX * numY), numParticles(numParticles), WIDTH(WIDTH), HEIGHT(HEIGHT), gravity(gravity), k(k), diffusionRatio(diffusionRatio), separationInit(separationInit), vorticityStrength(vorticityStrength_), flipRatio(flipRatio_), overRelaxation(overRelaxation_), numPressureIters(numPressureIters_), thread_pool(tp) {
            ...
            int temp = (numX) * (numY);
            this->direction.resize(temp);
            this->residual.resize(temp);
            this->pressure.resize(temp);
            this->residual.resize(temp);
            this->Ad.resize(temp);
        ...
        for (int i = 0; i < this->numX; ++i) {
            for (int j = 0; j < this->numY; ++j) {
                if (this->cellType[i * n + j] == SOLID_CELL) {
                    ...
                    this->cellType[i * n + j] = FLUID_CELL;//AIR_CELL;
    ...
    void PCGproject2() {
        ...
        //std::fill(begin(direction), end(direction), 0.0);

        setUpResidual();

        std::copy(begin(residual), end(residual), begin(direction));

        std::vector<float> uCopy(numX * numY);
        std::vector<float> vCopy(numX * numY);

        std::copy(begin(u), end(u), begin(uCopy));
        std::copy(begin(v), end(v), begin(vCopy));

        double sigma = Dot(residual, residual);
        
        for (int iter = 0; iter < 1 && sigma > 0; ++iter) {
            //std::cout << "iter: " << iter << ", before: " << calcDivergence(uCopy, vCopy) << ", ";
            ATimes();
            ...
            std::vector<float> uCopyCopy(numX * numY);
            std::vector<float> vCopyCopy(numX * numY);

            std::copy(begin(uCopy), end(uCopy), begin(uCopyCopy));
            std::copy(begin(vCopy), end(vCopy), begin(vCopyCopy));
            applyPressureToCopies(uCopyCopy, vCopyCopy);
            //std::cout << "after: " << calcDivergence(uCopyCopy, vCopyCopy) << "\n";
        }
    ...
    void setUpA() {
        ...
        for (int i = 1; i < numX - 1; ++i) {
            for (int j = 1; j < numY - 1; ++j) {
                ...
                Adiag[idx] += direction[idx];

                if (cellType[(i - 1) * n + j] == FLUID_CELL) {
                    Adiag[idx] -= direction[(i - 1) * n + j];//scale;
                }
                ...
                if (cellType[i * n + j - 1] == FLUID_CELL) {
                    Adiag[idx] -= direction[i * n + j - 1];//scale;
                }
    ...
    void render(sf::RenderWindow& window) {
        ...
        for (int i = 0; i < numX; ++i) {
            for (int j = 0; j < numY; ++j) {
                ...
                text.setString(std::to_string(div));
                window.draw(text);
            }
        }
